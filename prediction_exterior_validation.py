...
class regex():
    def __init__(self,path):
        ...
        self.goodDataPath = "Prediction_Batch_Files_validated/Good_Raw"
    ...
    def goodbad_del(self):
        ...
        try:
            path = 'Prediction_Batch_Files_validated/'
            ...
            if (re[0]+"_"+re[1])=='forest_cover':
                if len(re[2]) == LengthOfDateStampInFile:
                    if len(re[3]) == LengthOfTimeStampInFile:
                        shutil.copy("Prediction_Batch_files/" + f, "Prediction_Batch_Files_validated/Good_Raw")
                    else:
                        shutil.copy("Prediction_Batch_files/" + f, "Prediction_Batch_Files_validated/Bad_Raw")
                else:
                    shutil.copy("Prediction_Batch_files/" + f, "Prediction_Batch_Files_validated/Bad_Raw")
            else:
                shutil.copy("Prediction_Batch_files/" + f, "Prediction_Batch_Files_validated/Bad_Raw")
    ...
    def validate_columns(self,noofcolumns):
        ...
        try:
            for file in os.listdir('Prediction_Batch_Files_validated/Good_Raw/'):

                csv = pd.read_csv("Prediction_Batch_Files_validated/Good_Raw/" + file)
                if csv.shape[1] == noofcolumns:
                    pass
                else:
                    shutil.move("Prediction_Batch_Files_validated/Good_Raw/" + file, "Prediction_Batch_Files_validated/Bad_Raw")
        except Exception as e:
            raise e
    ...
    def create_good_bad(self):
        ...
        try:
            path = os.path.join("Prediction_Batch_Files_validated/", "Good_Raw/")
            if not os.path.isdir(path):
                os.makedirs(path)
            path = os.path.join("Prediction_Batch_Files_validated/", "Bad_Raw/")
            if not os.path.isdir(path):
                os.makedirs(path)
            ...
            for file in os.listdir('Prediction_Batch_Files_validated/Good_Raw/'):
                ...
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        count += 1
                        shutil.move("Prediction_Batch_Files_validated/Good_Raw/" + file,
                                    "Prediction_Batch_Files_validated/Bad_Raw")
                        break
                if count == 0:
                    csv.to_csv("Prediction_Batch_Files_validated/Good_Raw/" + file, index=None, header=True)
        except Exception as e:
            raise e
